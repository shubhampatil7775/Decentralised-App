...
class ViewRequestDetail extends Component {
      ...
      ViewRequestDetails=async()=> {
        ...
        var requestlistchild = document.getElementById("requestlistchild");
          ...
          this.state.sch.methods.ViewRequestDetail(this.state.currentuser,2).call({from:this.state.currentuser},(error,result)=>{
            ...
            this.state.rector = Number(result[2]);
            this.state.scholor = Number(result[3]);
            this.state.academics = Number(result[4]);
            this.state.dochash = Number(result[5]);
			...
			if(this.state.rector == 1)
			{
				var listHTML = "<tr><td width='80%'>Rector</td><td width='20%' align='center'><input type='checkbox' id='chk_rector'/></td></tr>";
				requestlistchild.insertAdjacentHTML('beforeend',listHTML);
			}
			...
			if(this.state.dochash == 1)
			{
				var listHTML = "<tr><td width='80%'>Dochash</td><td width='20%' align='center'><input type='checkbox' id='chk_dochash'/></td></tr>";
				requestlistchild.insertAdjacentHTML('beforeend',listHTML);
			}
    ...
    UpdateRequestStatu=async()=>
    {
        ...
        {
            AccessReqCount=AccessReqCount+1;
            if(document.getElementById("chk_rector").checked)
            {
                this.state.rector=2;
                AccessPerCount=AccessPerCount+1;
            }
        ...
        {
            AccessReqCount=AccessReqCount+1;
            if(document.getElementById("chk_dochash").checked)
            {
                this.state.dochash=2;
                AccessPerCount=AccessPerCount+1;
            }
            else
            {
                this.state.dochash=3;
            }
        }
